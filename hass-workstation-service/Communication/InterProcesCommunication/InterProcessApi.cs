...
namespace hass_workstation_service.Communication.InterProcesCommunication
{
	public class InterProcessApi : IServiceContractInterfaces
	{
		...
		public MqqtClientStatus GetMqqtClientStatus() {
			return _publisher.GetStatus();
		}

		public Task<MqttSettings> GetMqttBrokerSettings() {
			return _configurationService.GetMqttBrokerSettings();
		}
        ...
        public void WriteMqttBrokerSettingsAsync(MqttSettings settings) {
			_configurationService.WriteMqttBrokerSettingsAsync(settings);
		}
        ...
        public void EnableAutostart(bool enable) {
			_configurationService.EnableAutoStart(enable);
		}

		public bool IsAutoStartEnabled() {
			return _configurationService.IsAutoStartEnabled();
		}
		...
		public void RemoveSensorById(Guid id) {
			_configurationService.DeleteConfiguredSensor(id);
		}

		public void RemoveCommandById(Guid id) {
			_configurationService.DeleteConfiguredCommand(id);
		}
        ...
        /// <summary>
        ///     Adds a command to the configured commands. This properly initializes the class, subscribes to the command topic and
		...
		public Task<GeneralSettings> GetGeneralSettings() {
			return _configurationService.ReadGeneralSettings();
		}


		public void WriteGeneralSettings(GeneralSettings settings) {
			_configurationService.WriteGeneralSettingsAsync(settings);
		}
		...
		private AbstractSensor GetSensorToCreate(AvailableSensors sensorType, string json) {
			...
			var model = JsonSerializer.Deserialize<dynamic>(json, serializerOptions);

			return sensorType switch {
				AvailableSensors.UserNotificationStateSensor => new UserNotificationStateSensor(_publisher,
					(int)model.UpdateInterval, model.Name),
				AvailableSensors.DummySensor => new DummySensor(_publisher, (int)model.UpdateInterval, model.Name),
				AvailableSensors.CurrentClockSpeedSensor => new CurrentClockSpeedSensor(_publisher,
					(int)model.UpdateInterval, model.Name),
				AvailableSensors.CPULoadSensor => new CPULoadSensor(_publisher, (int)model.UpdateInterval, model.Name),
				AvailableSensors.WMIQuerySensor => new WMIQuerySensor(_publisher, model.Query,
					(int)model.UpdateInterval, model.Name, scope: model.Scope),
				AvailableSensors.MemoryUsageSensor => new MemoryUsageSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.ActiveWindowSensor => new ActiveWindowSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.WebcamActiveSensor => new WebcamActiveSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.WebcamProcessSensor => new WebcamProcessSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.MicrophoneActiveSensor => new MicrophoneActiveSensor(_publisher,
					(int)model.UpdateInterval, model.Name),
				AvailableSensors.MicrophoneProcessSensor => new MicrophoneProcessSensor(_publisher,
					(int)model.UpdateInterval, model.Name),
				AvailableSensors.NamedWindowSensor => new NamedWindowSensor(_publisher, model.WindowName, model.Name,
					(int)model.UpdateInterval),
				AvailableSensors.LastActiveSensor => new LastActiveSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.LastBootSensor => new LastBootSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.SessionStateSensor => new SessionStateSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.CurrentVolumeSensor => new CurrentVolumeSensor(_publisher, (int)model.UpdateInterval,
					model.Name),
				AvailableSensors.GPUTemperatureSensor => new GpuTemperatureSensor(_publisher,
					(int)model.UpdateInterval, model.Name),
				...
				AvailableSensors.ScreenshotSensor => new ScreenshotSensor(_publisher, model.Name,
					(int)model.UpdateInterval, model.ScaleFactor),
				_ => null
			};
		}
