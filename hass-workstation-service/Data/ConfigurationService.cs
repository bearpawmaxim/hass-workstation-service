...
namespace hass_workstation_service.Data
{
	public class ConfigurationService : IConfigurationService
	{
		...
		public ICollection<AbstractSensor> ConfiguredSensors { get; }
		public ICollection<AbstractCommand> ConfiguredCommands { get; }
		...
		public async void ReadSensorSettings(MqttPublisher publisher) {
			...
			using (var stream = new FileStream(Path.Combine(_path, SENSORS_SETTINGS_FILENAME), FileMode.Open)) {
				...
				stream.Close();
				SensorsSettingsFileLocked = false;
			}

			foreach (var configuredSensor in sensors) {
				AbstractSensor sensor = null;
				switch (configuredSensor.Type) {
					case "UserNotificationStateSensor":
						sensor = new UserNotificationStateSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "DummySensor":
						sensor = new DummySensor(publisher, configuredSensor.UpdateInterval, configuredSensor.Name,
							configuredSensor.Id);
						break;
					case "CurrentClockSpeedSensor":
						sensor = new CurrentClockSpeedSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "CPULoadSensor":
						sensor = new CPULoadSensor(publisher, configuredSensor.UpdateInterval, configuredSensor.Name,
							configuredSensor.Id);
						break;
					case "MemoryUsageSensor":
						if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
							sensor = new MemoryUsageSensor(publisher, configuredSensor.UpdateInterval,
								configuredSensor.Name, configuredSensor.Id);
						break;
					case "ActiveWindowSensor":
						sensor = new ActiveWindowSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "NamedWindowSensor":
						sensor = new NamedWindowSensor(publisher, configuredSensor.WindowName, configuredSensor.Name,
							configuredSensor.UpdateInterval, configuredSensor.Id);
						break;
					case "LastActiveSensor":
						sensor = new LastActiveSensor(publisher, configuredSensor.UpdateInterval, configuredSensor.Name,
							configuredSensor.Id);
						break;
					case "LastBootSensor":
						sensor = new LastBootSensor(publisher, configuredSensor.UpdateInterval, configuredSensor.Name,
							configuredSensor.Id);
						break;
					case "WebcamActiveSensor":
						sensor = new WebcamActiveSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "WebcamProcessSensor":
						sensor = new WebcamProcessSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "MicrophoneActiveSensor":
						sensor = new MicrophoneActiveSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "MicrophoneProcessSensor":
						sensor = new MicrophoneProcessSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "SessionStateSensor":
						sensor = new SessionStateSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "CurrentVolumeSensor":
						sensor = new CurrentVolumeSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "MasterVolumeSensor":
						sensor = new MasterVolumeSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "GpuTemperatureSensor":
						sensor = new GpuTemperatureSensor(publisher, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id);
						break;
					case "GpuLoadSensor":
					...
					case "ScreenshotSensor":
						sensor = new ScreenshotSensor(publisher, configuredSensor.Name,
							configuredSensor.UpdateInterval, configuredSensor.Id, configuredSensor.ScaleFactor);
						break;
					// keep this one last!
					case "WMIQuerySensor":
						sensor = new WMIQuerySensor(publisher, configuredSensor.Query, configuredSensor.UpdateInterval,
							configuredSensor.Name, configuredSensor.Id, configuredSensor.Scope);
						break;
					default:
				...
				if (sensor != null) ConfiguredSensors.Add(sensor);
		...
		public async void ReadCommandSettings(MqttPublisher publisher) {
			...
			foreach (var configuredCommand in commands) {
				...
				switch (configuredCommand.Type) {
					...
					case "LogOffCommand":
						...
						break;
					case "CustomCommand":
						command = new CustomCommand(publisher, configuredCommand.Command, configuredCommand.Name,
							configuredCommand.Id);
						break;
					...
					case "MuteCommand":
						...
						break;
					case "KeyCommand":
						command = new KeyCommand(publisher, configuredCommand.KeyCode, configuredCommand.Name,
							configuredCommand.Id);
						break;
					default:
				...
				if (command != null) ConfiguredCommands.Add(command);
			}
		}
		...
		public async Task<GeneralSettings> ReadGeneralSettings() {
			...
			using (var stream = new FileStream(Path.Combine(_path, GENERAL_SETTINGS_FILENAME), FileMode.Open)) {
				...
				GeneralSettingsFileLocked = false;
				return settings;
			}
		}
        ...
        public async void WriteGeneralSettingsAsync(GeneralSettings settings) {
			...
			if (settings.NamePrefix != GeneralSettings.NamePrefix) {
				// notify the mqtt publisher of the new prefix
				NamePrefixChangedHandler.Invoke(settings.NamePrefix);
		...
		public async Task<IManagedMqttClientOptions> GetMqttClientOptionsAsync() {
			var configuredBroker = await ReadMqttSettingsAsync();
			if (configuredBroker != null && configuredBroker.Host != null) {
				...
				var tlsParameters = new MqttClientOptionsBuilderTlsParameters {
					...
					SslProtocol = configuredBroker.UseTLS ? SslProtocols.Tls12 : SslProtocols.None
				};
...
#pragma warning disable CA1416 // Validate platform compatibility. We ignore it here because this would never happen. A cleaner solution may be implemented later.
							configuredSensorsToSave.Add(new ConfiguredSensor {
								Id = wmiSensor.Id, Name = wmiSensor.Name, Type = wmiSensor.GetType().Name,
								UpdateInterval = wmiSensor.UpdateInterval, Query = wmiSensor.Query,
...
#pragma warning restore CA1416 // Validate platform compatibility
						...
						case NamedWindowSensor namedWindowSensor:
							configuredSensorsToSave.Add(new ConfiguredSensor {
								...
								Type = namedWindowSensor.GetType().Name,
								UpdateInterval = namedWindowSensor.UpdateInterval,
								WindowName = namedWindowSensor.WindowName
							});
							break;
						case ScreenshotSensor screenshotSensor:
							configuredSensorsToSave.Add(new ConfiguredSensor {
								Id = screenshotSensor.Id,
								Name = screenshotSensor.Name,
								Type = screenshotSensor.GetType().Name,
								UpdateInterval = screenshotSensor.UpdateInterval,
        ...
        public async void WriteMqttBrokerSettingsAsync(MqttSettings settings) {
			...
			using (var stream = new FileStream(Path.Combine(_path, MQTT_SETTINGS_FILENAME), FileMode.Open)) {
				...
				var configuredBroker = new ConfiguredMqttBroker {
					Host = settings.Host,
			...
			BrokerSettingsFileLocked = false;
			MqqtConfigChangedHandler.Invoke(await GetMqttClientOptionsAsync());
		}

		public async Task<MqttSettings> GetMqttBrokerSettings() {
			var broker = await ReadMqttSettingsAsync();
        ...
        /// <summary>
        ///     Enable or disable autostarting the background service. It does this by adding the application shortcut (appref-ms)
        ///     to the registry run key for the current user
        /// </summary>
		...
		public async Task<ICollection<AbstractSensor>> GetSensorsAfterLoadingAsync() {
			while (_sensorsLoading) await Task.Delay(500);
